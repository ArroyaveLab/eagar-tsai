# Contributing to eagar-tsai

Thanks for your interest in improving `eagar-tsai`. This guide walks through setting up a development environment and the checks your changes should pass before you open a pull request.

## Getting started

`eagar-tsai` uses [uv](https://docs.astral.sh/uv/) for dependency management and packaging.

```sh
git clone https://github.com/ArroyaveLab/eagar-tsai.git
cd eagar-tsai
...
Tests run against Python 3.12 through 3.14 in CI on Ubuntu, macOS, and Windows (see `.github/workflows/test.yml`); running locally on whatever interpreter `uv` picks up is fine. Slow tests are marked with `slow` and can be skipped locally with `-m "not slow"`.
...
- Write clear, imperative commit messages (e.g. "Add substrate preheat option", not "Added" or "Adding").
...
- Ensure `uv run prek run --all-files` and `uv run pytest tests/ -v` pass locally before opening the PR. CI runs the same checks (`.github/workflows/lint.yml` and `.github/workflows/test.yml`) and must pass before merge.

## Reporting bugs and requesting features

Please use the GitHub issue tracker:

- [Report a bug](https://github.com/ArroyaveLab/eagar-tsai/issues/new?labels=bug)
- [Request a feature](https://github.com/ArroyaveLab/eagar-tsai/issues/new?labels=enhancement)

Include a minimal reproducible example when reporting a bug, ideally with the input parameters and a small snippet that triggers the issue.
